...
# 300 x 60, N_Pts x N_modes
'''
GEG_k = np.zeros((300,61))
LEG_k = np.zeros((300,61))
GEG_k1 = np.zeros((300,61))
LEG_k1 = np.zeros((300,61))
...
def Outer_Prod(R,theta,N_modes,X,d,Re_1,Re_2,Pr): # Fix this code to extend to the boundaries
...
def SPEC_TO_GRID(R,xx,theta,N_modes,X,d,Re_1,Re_2,Pr): # Fix this code to extend to the boundaries
...
def Plot_Package_CHE(R,theta,omega,psi,thermal,sigma,Re1,Ra,Pr): # Returns Array accepted by contourf - function
	...
	ax[0].set_title(r'$\Omega$', fontsize=16, va='bottom')
...
def	VID(R,theta,N_Modes,X_VID,d,Re_1,Re_2,Ra,dt):
	sigma = d;
	alpha = 1.0+d;
	#-- Make Really Narrow Slice to form Axis ---------------
	NN = 20;
	...
	RR = R[1:-1];
	for k in range(len(X_VID[0,:])): # Evolve For the number of time steps evaluated
